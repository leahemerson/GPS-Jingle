...
class Geopoint(object):
  ...
  def __init__(self, latitude, longitude, elevation, elevationConverter=1.0):
    self.latitude = float(latitude)
    self.longitude = float(longitude)
    self.elevation = float(elevation) * elevationConverter
    self.waypoint = None
    self.distanceToWaypoint = sys.maxsize
...
class Waypoint(Geopoint):
  def __init__(self, latitude, longitude, elevation, elevationConverter=1.0, 
    ...
    Geopoint.__init__(self, latitude, longitude, elevation, elevationConverter)
    self.name = name
    self.type = type
